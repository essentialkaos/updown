...
func (s *UpdownSuite) TestBasicErrors(c *C) {
	...
	var cc *Check
	c.Assert(cc.Link(), Equals, "https://updown.io")
}
...
func (s *UpdownSuite) TestGetChecks(c *C) {
	...
	c.Assert(chk.IsDown, Equals, false)
	c.Assert(chk.UpSince.Unix(), Equals, int64(1703322411))
	c.Assert(chk.Error, Equals, "")
	c.Assert(chk.Period, Equals, 15)
	c.Assert(chk.Apdex, Equals, 0.5)
	c.Assert(chk.StringMatch, Equals, "")
	c.Assert(chk.IsEnabled, Equals, true)
	c.Assert(chk.IsPublished, Equals, true)
	c.Assert(chk.DisabledLocations, HasLen, 0)
	c.Assert(chk.Recipients, HasLen, 2)
	c.Assert(chk.LastCheckAt.Unix(), Equals, int64(1639717201))
	c.Assert(chk.NextCheckAt.Unix(), Equals, int64(1639717216))
	c.Assert(chk.CreatedAt.Unix(), Equals, int64(1348320584))
	c.Assert(chk.MuteUntil.IsZero(), Equals, true)
	c.Assert(chk.FaviconURL, Equals, "https://updown.io/favicon.png")
	c.Assert(chk.CustomHeaders, HasLen, 0)
	c.Assert(chk.HTTPVerb, Equals, "GET/HEAD")
	c.Assert(chk.HTTPBody, Equals, "")
	c.Assert(chk.SSL, NotNil)
	c.Assert(chk.SSL.TestedAt.Unix(), Equals, int64(1639717084))
...
func (s *UpdownSuite) TestGetCheck(c *C) {
	...
	chk, err := api.GetCheck("ngg8", true)
